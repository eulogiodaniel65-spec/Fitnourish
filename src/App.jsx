...
const mkExercise = (id, name, sets, reps, targetWeight, restSets, restAfter, mediaLabel, comments = []) => ({
  id, name, sets, reps, targetWeight, restSets, restAfter, mediaLabel,
...
function MediaBox({ label }) {
  return (
    ...
    >
      ...
      <span style={{ fontFamily: "Inter", fontSize: 12, color: COLORS.dim }}>{label}</span>
    </div>
  );
}
...
function ProfesorView({ alumnos, setAlumnos, usuario }) {
  ...
  const [newExercise, setNewExercise] = useState({ dayIdx: 0, name: "", sets: "3", reps: "10", targetWeight: "", restSets: "60", restAfter: "90" });
  ...
  const addExercise = async () => {
    if (!newExercise.name.trim() || !selected) return;
    const day = selected.days[newExercise.dayIdx];
    try {
      const nuevo = await api.agregarEjercicioADia({
        dayId: day.dayId,
      ...
      const exerciseObj = mkExercise(
        nuevo.id,
        nuevo.ejercicios?.nombre || newExercise.name,
        nuevo.series,
        nuevo.reps,
        nuevo.peso_objetivo,
        nuevo.descanso_series_seg,
        nuevo.descanso_posterior_seg,
        nuevo.ejercicios?.url_media || "Sin media"
      );
      ...
      setNewExercise({ ...newExercise, name: "", targetWeight: "" });
    } catch (err) {
          ...
          <div style={{ background: COLORS.surface, border: `1px dashed ${COLORS.border}`, borderRadius: 10, padding: 16 }}>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <input
                ...
                onChange={(ev) => setNewExercise({ ...newExercise, name: ev.target.value })}
